...
from app.models import User, Role, Category, SubCategory, Venue, Event, Seat, Payment, Order, Ticket, UserEvent
from app.crud import (
    create_user, get_user_by_id, get_all_users, delete_user,
    create_role, get_all_roles, get_role_by_id,delete_role,
    create_category, get_all_categories,get_category_by_id, delete_category,
    create_subcategory, get_all_subcategories, get_subcategory_by_id,delete_subcategory,
    create_venue, get_all_venues, get_venue_by_id, delete_venue,
    create_event, get_event_by_id, delete_event, 
    create_seat, get_all_seats, get_seat_by_id, delete_seat,
    create_payment, get_payment_by_id, delete_payment,
    create_order, get_order_by_id, delete_order,
    create_ticket, get_all_tickets, delete_ticket, get_ticket_by_id,
    add_user_event, get_user_events_by_user_id,
    update_user, update_role, update_category, update_subcategory,
    update_venue, update_event, update_seat, update_payment,
    update_order, update_ticket, update_user_event
)
...
@router.get("/roles/{role_id}", response_model=User)
...
def api_update_category(category_id: int, category: Category, session: Session = Depends(get_session)):
...
def api_create_subcategory(subcategory: SubCategory, session: Session = Depends(get_session)):
...
def api_update_subcategory(subcategory_id: int, subcategory: SubCategory,
                           session: Session = Depends(get_session)):
    return update_subcategory(session ,subcategory_id ,subcategory )

@router.delete("/subcategories/{subcategory_id}")
def api_delete_subcategory(subcategory_id: int, session: Session = Depends(get_session)):
...
@router.get("/venues/{venues_id}", response_model=Venue)
...
def api_update_venue (venue_id :int ,venue :Venue ,session :Session =Depends (get_session )):
...
def api_update_event (event_id :int ,event :Event ,session :Session =Depends (get_session )):
...
def api_update_payment(payment_id: int, payment: Payment,
                       session: Session = Depends(get_session)):
...
def api_update_order(order_id: int, order: Order,
                     session: Session = Depends(get_session)):
...
def api_update_ticket(ticket_id: int, ticket: Ticket,
                      session: Session = Depends(get_session)):
